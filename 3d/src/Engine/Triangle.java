...
public class Triangle implements Cloneable,Comparable<Triangle>{
	...
	public int compareTo(Triangle o) {
		...
		List<Vector> vectors_first_triangle=new ArrayList<Vector>();	// Vectors to points contained by the first triangle
		List<Vector> vectors_second_triangle=new ArrayList<Vector>();	// Vectors to points contained by the second triangle
		List<Point> res=getEquidistantPoints();
		
		for(Point temp:res) {
			vectors_first_triangle.add(new Vector(temp));
		}
		res=o.getEquidistantPoints();
		for(Point temp:res) {
		...
		for(Vector line:vectors_first_triangle) {
			...
			if(intersection!=null && o.isPointInside(intersection)) {
				...
				double intersect_dist=Math.sqrt(intersection.x*intersection.x+intersection.y*intersection.y+intersection.z*intersection.z);
				if(!doubleEq(dist - intersect_dist,0)) order+=-Double.compare(dist, intersect_dist);
			}
		}
		...
		for(Vector line:vectors_second_triangle) {
			...
			if(intersection!=null && isPointInside(intersection)) {
				...
				double dist=Math.sqrt(line.x*line.x+line.y*line.y+line.z*line.z);
				double intersect_dist=Math.sqrt(intersection.x*intersection.x+intersection.y*intersection.y+intersection.z*intersection.z);
				if(!doubleEq(dist - intersect_dist,0)) order+= -Double.compare(intersect_dist, dist);
			}
		}

		if(order==0)return -Double.compare(center_dist1,center_dist2);
	...
	public List<Point>  getEquidistantPoints() {
		...
		// The formula works out to be: 4*( 3^level-1)/(3-1)  --there's a geometric sum involved
		double necesary_points=area*points_per_area;
	...
	// Level means how many times to reapply the algorithm for succesive sub-triangles for better coverage
